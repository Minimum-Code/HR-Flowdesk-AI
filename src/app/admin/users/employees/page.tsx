...
export default function AdminEmployeesPage() {
  ...
  function load() {
    setLoading(true)
    adminApi.get('/admin/users/employees')
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      await adminApi.post('/admin/users', { ...form, role: 'employee' })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#07472e]">Employees</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="bg-[#07472e] text-[#fbfff3] hover:bg-[#05351f] font-semibold">+ Add Employee</Button>
          </DialogTrigger>
          <DialogContent className="bg-[#ffffff] border-[#07472e]/10 text-[#07472e]">
            <DialogHeader><DialogTitle className="text-[#07472e]">Add Employee</DialogTitle></DialogHeader>
            <form onSubmit={handleCreate} className="space-y-4 mt-2">
              <div className="space-y-1">
                <Label className="text-[#647a6b]">Full Name</Label>
                <Input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  className="border-[#07472e]/20 bg-[#07472e]/5 text-[#07472e] placeholder:text-[#647a6b]" placeholder="John Employee" />
              </div>
              <div className="space-y-1">
                <Label className="text-[#647a6b]">Email</Label>
                <Input required type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                  className="border-[#07472e]/20 bg-[#07472e]/5 text-[#07472e] placeholder:text-[#647a6b]" placeholder="john@acme.com" />
              </div>
              <div className="space-y-1">
      ...
      <div className="rounded-[32px] bg-[#ffffff] p-6">
        {loading ? (
          ...
          <p className="py-10 text-center text-[#647a6b]">No employees found</p>
        ) : (
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="bg-[#ffffff] border-[#07472e]/10 text-[#07472e]">
          <DialogHeader><DialogTitle className="text-[#07472e]">Edit Employee</DialogTitle></DialogHeader>
