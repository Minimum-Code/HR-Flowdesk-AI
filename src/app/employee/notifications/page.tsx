...
export default function EmployeeNotificationsPage() {
  ...
  async function load() {
    try {
      const res = await employeeApi.get('/employee/notifications', { params: { page: 1, per_page: 50 } })
  ...
  async function markAllRead() {
    try {
      await employeeApi.patch('/employee/notifications/read-all')
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {notifications.map((n) => (
              <div
                key={n.id}
                className={`rounded-2xl px-5 py-4 transition-colors ${n.is_read ? 'bg-[#fbfff3]' : 'bg-[#07472e]/10 border border-[#07472e]/20'}`}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  {!n.is_read && (
                    <div className="mt-1 h-2 w-2 shrink-0 rounded-full bg-[#07472e]" />
                  )}
                </div>
              </div>
            ))}
