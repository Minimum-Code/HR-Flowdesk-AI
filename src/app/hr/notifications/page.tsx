...
export default function HRNotificationsPage() {
  ...
  useEffect(() => {
    hrApi.get('/hr/notifications', { params: { page: 1, per_page: 50 } })
  ...
  async function markAllRead() {
    try {
      await hrApi.patch('/hr/notifications/read-all')
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {unread > 0 && <p className="text-[#647a6b] mt-1">{unread} unread</p>}
        </div>
        {unread > 0 && (
      ...
      <div className="rounded-[32px] bg-[#ffffff] p-6">
        {loading ? (
          ...
          <div className="space-y-2">
            {notifications.map((n) => (
              <div key={n.id} className={`rounded-2xl px-5 py-4 ${n.is_read ? 'bg-[#fbfff3]' : 'bg-[#07472e]/10 border border-[#07472e]/20'}`}>
